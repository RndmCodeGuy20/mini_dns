...
select() wakes on listen socket readable
  ...
  → record_store().find()  (locked, case-insensitive scan of the record table)
      match, qtype==A,    entry has ipv4 → build_a_record_response()      → sendto()
      match, qtype==AAAA, entry has ipv6 → build_aaaa_record_response()   → sendto()
      match, family absent for this qtype→ build_nodata_response()  (Phase 6: name
                                             exists, NOERROR/no answers — not NXDOMAIN)
...
select() wakes on forwarder socket readable
  → DnsForwarder::handle_upstream_readable()
      (matches reply to its in-flight slot via slot-index + generation in the
       transaction ID — accepting a reply from either upstream, Phase 6 — scans
       the answer section for min TTL)
...
select() times out, or every loop iteration regardless of wake reason
  → DnsForwarder::reap_expired()     (in-flight queries past their deadline)
      attempt==0 (primary only)  → resend to secondary upstream, same slot,
                                     fresh deadline (Phase 6) — stays in-flight
      attempt==1 (secondary too) → build_relayed_response(SERVFAIL) → sendto()
...
- `POST`/`PUT`/`DELETE /api/records` (Phase 5) → `add_cors_headers()` (reflects `Origin`) → `check_auth()` (Basic auth against `admin_credentials.h`; 401 + `WWW-Authenticate` on failure) → parses/validates a `{"host","ip"?,"ipv6"?}` JSON body (Phase 6: `ip`/`ipv6` are each optional, but create/update reject a body with neither) → `record_store().create()`/`update()`/`remove()`, mapped to `201`/`200`/`400`/`404`/`507`. `PUT` is a full replace of the address set, not a per-family patch. `OPTIONS /api/records` answers CORS preflight (204, no auth). See the Phase 5 design doc for the original validation/status-code mapping and the Phase 6 doc for the dual-stack extension.
...
- **NODATA, not NXDOMAIN, for a wrong-family match (Phase 6)** — querying `AAAA` for a name that only has an A record (or vice versa) returns the RFC-correct NODATA (NOERROR + empty answers), not NXDOMAIN. This reversed a Phase 1–5 simplification: once a name can legitimately hold only one family, telling a dual-stack client the name doesn't exist at all (NXDOMAIN) would make it stop trying the family that *does* exist. `DnsRecordStore::find()` returning the whole entry (not just one address) is what makes this distinguishable — see `dns_record_store.h`.
